...
for subject in subjects:
    for iter, lbl in enumerate(labels):
        ...
        ds = source.make_stc_epochs(meg_ds, reject=3e-12, label=lbl, from_file=True)

#This z-scores your data for each subject separately 
        ds['stc'].x = stats.mstats.zscore(ds['stc'].x)

        ds['label'] = E.factor([ds.info['label']], rep=ds.N)

        datasets.append(ds)
        
group_ds = E.combine(datasets)
group_ds.info = ds.info['raw'].info

sfreq = int(group_ds.info['sfreq'][0])

start = abs(group_ds['stc'].time.x[0]*sfreq)
time = group_ds['stc'].time.x[start:]
...
for effect, title in zip(effects, titles):
    ...
    for i, lbl in enumerate(labels):
        ...
        if any(index) == True:
            ...
            pyplot.savefig(os.path.join(os.path.expanduser('~'), 'Dropbox', 'Experiments', 'NMG', 'write-up', 'corrs', '%s-%s.png' %(lbl, effect)))
        else:
            continue
