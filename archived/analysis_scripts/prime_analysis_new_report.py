...
# Create a report
report = Report(title="Prime Analyses")
...
for wtype in ['novel']:
    ...
    for i, cluster in enumerate(a.clusters[a.clusters['p'] < .1].itercases()):
        ...
        image = E.plot.brain.image(plt_extent, "cluster %s extent.png" % i,
                                   alt=None, close=True)
        image.savefig(e.get('plot-file', analysis=wtype+'_brain'))
...
# save the report
report.save_html(e.get('report-file', analysis=analysis + '_temporal_final'))
